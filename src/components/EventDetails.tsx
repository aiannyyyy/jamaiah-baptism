  ...
  return (
    <section className="px-5 pb-4 space-y-3">
      ...
      <DetailCard icon={<Church className="w-5 h-5" />}>
        <p className="font-semibold mb-1">Gabbie's Baptism</p>
        <div className="flex items-center gap-1.5 text-sm text-blush-600">
      ...
      <DetailCard icon={<Utensils className="w-5 h-5" />}>
        <p className="font-semibold mb-1">Reception</p>
        <div className="flex items-center gap-1.5 text-sm text-blush-600">
          <Clock className="w-3.5 h-3.5 shrink-0" />
...
function VenueLink({ name, url }: { name: string; url: string }) {
  const linkClass = "text-sm text-blush-600 mt-1 inline-flex items-start gap-1 hover:text-blush-500 transition-colors group";
